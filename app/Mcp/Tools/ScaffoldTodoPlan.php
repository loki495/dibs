...
#[Description('Creates a plan issue together with its initial child task issues in one atomic call: if any child fails validation, nothing is created at all — never a plan with a partial set of children. Each child is scoped to the same area as the plan. Apply the "plan" label via labelIds if the plan should read as a maintained plan document. Pass idempotencyKey to make a retried call return the original plan instead of duplicating it.')]
