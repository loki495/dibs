...
- Livewire components and Artisan commands delegate business logic to typed Actions, shared by the UI and the MCP/CLI agent surface. Actions write SQLite directly as the confirmed result and enqueue a GitHub push rather than calling the GitHub API inline. Actions may chain other Actions (e.g. `UpdateTodoIssue` builds on `ResolveLabels`, `MoveIssueUnderParent`, `AssignIssueToProject`), and the UI, the MCP tools and `app/Http` must never write to the database themselves — `tests/Feature/Architecture/NoDirectWritesInUiOrMcpTest.php` fails on any Eloquent/query-builder write or transaction in those layers.
...
The canonical agent interface is a host-local stdio MCP server (`App\Mcp\Servers\TodoServer`, started with `php artisan mcp:start todo`). It exposes 18 tools — context/read (`todo_status`, `todo_context`, `todo_metadata`, `todo_list`, `todo_search`, `todo_show`, `todo_queue_status`), create/revise/comment (`todo_create`, `todo_scaffold_plan`, `todo_revise`, `todo_comment`), the full claim lifecycle (`todo_claim`, `todo_heartbeat`, `todo_release`, `todo_complete`, `todo_claim_status`), reopening a closed task (`todo_reopen`, not claim-scoped), and a tooling self-report tool (`todo_report_bug`) — all delegating to the same typed Actions the web UI uses. Every write tool validates its own arguments explicitly and implements `Laravel\Mcp\Server\Contracts\Errable`: `laravel/mcp` does not enforce a tool's declared JSON Schema before invoking its handler, so schema-shaped input alone is not a safety guarantee.

`todo_search` is the discovery entry point when an agent knows a topic: it searches titles and bodies across tasks, plans, and knowledge, including closed records by default, and returns ranked summaries with bounded body excerpts. Use `todo_list` for structured task browsing and `todo_show` for selected full records. `todo_search` and `todo_list` share one filter set (`App\Support\IssueFilters`, applied by `ApplyIssueFilters`): all/any/excluded labels, several Projects and Groups by id or name, and a parent's direct children or whole tree; the search `query` is optional when a filter is given, and a name that matches nothing comes back under `unresolved` rather than as an error. The complete matching and response contract is in `docs/agent-interface.md`.
...
`/activity` (`resources/views/pages/activity.blade.php`) browses the two activity logs: `mcp_call_logs` (every MCP tool call, arguments redacted and truncated) and `change_logs` (Action-level data changes with field diffs). Write Actions record their own entries through `ActivityRecorder`; there are no model observers, and the page reads through `ListActivityLog`/`DescribeActivityFilterOptions`/`CountRelatedActivity` and clears through `ClearActivityLog`, never writing itself. Retention comes from `DIBS_ACTIVITY_*_RETENTION_DAYS` and `activity:prune` runs daily. See `docs/architecture.md` for the design and what is not yet instrumented.

## Claims and coordination

A live claim binds to the caller's real OS process (host, pid, process start time verified via `/proc/<pid>/stat`), not a self-reported session string, plus a server-issued capability token required for every subsequent heartbeat/release/complete on that claim. A PID that's since been reused by an unrelated process cannot keep an old claim valid; a claim whose process is confirmed dead becomes automatically recoverable by the next claim attempt. This contract is scoped to one local database — it does not arbitrate ownership across independent databases or users; multi-instance/multi-user collaboration is an intentionally open future direction, not yet designed. Completing a task (`CloseTodoIssue`/`CompleteTodoTask`) can carry a closing note (a `Comment::KIND_CLOSING` comment), a reason (`COMPLETED`/`NOT_PLANNED`, GitHub's own `IssueClosedStateReason`, pushed with the close), and references, all surfaced back via `todo_show`'s `closing` object. See `docs/agent-interface.md` for the full model, including the workspace UI's claim/plan visibility (live claim ownership, a human-authorized "Release claim" override, and parent/child/knowledge navigation).
