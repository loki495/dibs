...
`todo_search` is the discovery entry point when an agent knows a topic: it searches titles and bodies across tasks, plans, and knowledge, including closed records by default, and returns ranked summaries with bounded body excerpts. Use `todo_list` for structured task browsing and `todo_show` for selected full records. Search accepts `query` plus optional area/group/label/state and pagination filters; the complete matching and response contract is in `docs/agent-interface.md`.
