...
it('renews a live claim and prints the result as json', function (): void {
    $issue = Issue::factory()->create();
    $result = app(ClaimTaskForAgent::class)->handle($issue, 'codex', posix_getppid(), 5);

    $exitCode = Artisan::call('todo:agent:heartbeat', [
...
it('rejects a missing --pid without crashing', function (): void {
    ...
    $exitCode = Artisan::call('todo:agent:heartbeat', ['issue' => (string) $issue->id, '--token' => 'x']);
...
it('rejects a missing --token without crashing', function (): void {
    ...
    $exitCode = Artisan::call('todo:agent:heartbeat', ['issue' => (string) $issue->id, '--pid' => (string) posix_getppid()]);
...
it('surfaces a domain exception for a wrong token as a clean error', function (): void {
    ...
    expect($exitCode)->toBe(1)
        ->and(TaskClaim::query()->sole()->expires_at)->not->toBeNull();
});
