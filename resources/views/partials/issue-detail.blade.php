    ...
    <section class="absolute inset-y-0 right-0 flex w-full max-w-2xl flex-col bg-white shadow-2xl dark:bg-slate-900">
        ...
        <div class="flex-1 space-y-5 overflow-y-auto p-5 pb-[calc(1.25rem+env(safe-area-inset-bottom,0px))]">
            ...
            <section class="border-t border-slate-200 pt-5 dark:border-slate-800">
                ...
                <form wire:submit="addComment" class="mt-5 space-y-3"><flux:textarea wire:model="newCommentBody" label="{{ __('Add a comment') }}" rows="4" placeholder="{{ __('Add context, a result, or a follow-up') }}" />@error('newCommentBody')<p role="alert" class="text-sm text-amber-700 dark:text-amber-400">{{ $message }}</p>@enderror@if ($commentError)<p role="alert" class="text-sm text-amber-700 dark:text-amber-400">{{ $commentError }}</p>@endif<div class="flex items-center justify-end gap-2">@if ($detail['issue']->state === 'OPEN')<flux:button type="button" wire:click="closeWithComment('{{ \App\Actions\CloseTodoIssue::REASON_COMPLETED }}')" wire:confirm="{{ __('Close this task? Any text in the comment box is saved as the closing note.') }}" variant="ghost" size="sm" class="text-red-600! hover:bg-red-50! dark:text-red-400! dark:hover:bg-red-950/40!">{{ __('Close') }}</flux:button>@else<flux:button type="button" wire:click="reopenIssue" wire:confirm="{{ __('Reopen this task?') }}" variant="ghost" size="sm">{{ __('Reopen') }}</flux:button>@endif<flux:button type="submit" size="sm">{{ __('Add comment') }}</flux:button></div></form>
            </section>
        </div>
    </section>
</div>
